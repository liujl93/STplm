...
// [[Rcpp::export]]
Eigen::MatrixXd nonparlogprof(const Eigen::VectorXd theta0,
    ...
    double sigma2 = (Zprime.adjoint())*(psi.llt().solve(Zprime));
    sigma2 = sigma2/Pd;
...
/*** R
 ...
 system.time(test <- logProfileCpp(theta0, CovStructure,loctim,DS,DT,Xprime,Zprime))
 system.time(testR <- log.profile(theta0, CovStructure,loctim,DS,DT,P,Xprime,Zprime))
 test == testR
 */
